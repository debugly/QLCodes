...
/**
 * 解压缩图像:Decompressing images that are downloaded and cached can improve peformance but can consume lot of memory.
...
/**
 * 添加一个缓存搜索路径：Add a read-only cache path to search for images pre-cached by SDImageCache
...
/**
 * 检索图片，检索到了会读取：Query the disk cache asynchronously.
 *
...
/**
 *  检索图片存在与否，不会读取图片Async check if image exists in disk cache already (does not load the image)
 *
